...
class GameLogic:
    def __init__(self):
        ...
        self.review_album = []  # Album cho từ cần ôn tập
        self.current_word = ""
        self.current_info = ""
        self.filtered_words = []
        self.score = 0
...
class HomePage:
    ...
    def setup_ui(self):
        with ui.row().classes('w-full items-center gap-4 mb-6'):
            ...
            ui.label('FLASHCARDS').classes('text-2xl font-bold text-indigo-600') 
        ...
        with ui.row().style('justify-content: center; margin: 10px 0;gap: 10px; flex-wrap: wrap;'): 
            ui.link('Flashcards Study', '/flashcards').classes(
                'w-full bg-indigo-600 hover:bg-indigo-800 text-white font-semibold py-2 rounded-lg shadow-md text-center no-underline'
            )
...
class GameUI:
    ...
    def setup_ui(self):
        ...
        with ui.row().classes('w-full items-center gap-4 mb-6'):
            ...
            ui.label('Word Scramble Game').classes('text-2xl font-bold text-indigo-600')
...
class ReviewUI:
    ...
    def setup_ui(self):
        with ui.row().classes('w-full items-center gap-4 mb-6'):
            ...
            ui.label('Review Cards').classes('text-2xl font-bold text-indigo-600')
...
@ui.page('/flashcards')
def flashcards():
    game_logic = GameLogic()  # Assuming this class exists
    review_logic = ReviewLogic(game_logic)  # Assuming this class exists
    with ui.column().classes('w-full max-w-3xl mx-auto p-4'):
        FlashcardsUI(game_logic, review_logic).setup_ui()
...
def game():
    ...
    review_logic = ReviewLogic(game_logic)  # Assuming this class exists
    with ui.column().classes('w-full max-w-3xl mx-auto p-4'):
        GameUI(game_logic, review_logic).setup_ui()
...
def review():
    ...
    review_logic = ReviewLogic(game_logic)  # Assuming this class exists
    with ui.column().classes('w-full max-w-3xl mx-auto p-4'):
        ReviewUI(game_logic, review_logic).setup_ui()
